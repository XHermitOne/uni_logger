{
Модуль узла OPC HDA сервера. Работа через обертку GrayBox HDA DLL.

Версия: 0.0.1.1
}

unit opc_gb_hda_node;
...
const
  OPC_GB_HDA_NODE_TYPE: AnsiString = 'OPC_GB_HDA';
  ...
  DLL_NAME: AnsiString = 'gbhda_aw.dll';


type
  {
  Класс взаимодействия с OPC HDA сервером. Работа через обертку GrayBox HDA DLL.
  }
  TICGrayBoxOPCHDANode = class(TICObjectProto)
...
constructor TICGrayBoxOPCHDANode.Create;
begin
  inherited Create;
...
destructor TICGrayBoxOPCHDANode.Destroy;
begin
  inherited Destroy;
...
{ Установить наименование OPC сервера }
procedure TICGrayBoxOPCHDANode.SetOPCServerName(sName: AnsiString);
begin
  FOPCServerName := sName;
end;

{ Установить наименование компьютера }
//procedure TICGrayBoxOPCHDANode.SetComputerName(sName: AnsiString);
//begin
//  FComputerName := sName;
//end;
...
procedure TICGrayBoxOPCHDANode.SetPropertiesArray(aArgs: Array Of Const);
begin
...
function TICGrayBoxOPCHDANode.ReadAll(dtTime: TDateTime = 0): TStringList;
var
//  HRes: HRESULT;
...
procedure TICGrayBoxOPCHDANode.SetProperties(dProperties: TStrDictionary);
var
  value: AnsiString;
begin
...
{  Установить связь }
function TICGrayBoxOPCHDANode.Connect(sComputer: AnsiString; sOPCServerName: AnsiString): Boolean;
//var
//  HRes: HRESULT;
...
{ Разорвать связь }
function TICGrayBoxOPCHDANode.Disconnect(): Boolean;
//var
//  HRes: HRESULT;
begin
  if FDllInstance <> 0 then
...
//function TICGrayBoxOPCHDANode.GetItemServerHandle(aServerInterface: IUnknown; sItem: String; iClient: DWORD; var iServer: DWORD): HRESULT;
//var
//  // ItemId элемента
//  sItemW: WideString;
//  PsItemW: POleStr;
...
{ Выбрать описания тегов из свойств }
function TICGrayBoxOPCHDANode.CreateTags(bClearValue: Boolean): TStrDictionary;
var
  i: Integer;
...
function TICGrayBoxOPCHDANode.CalcStartDateTime(dtEnd: TDateTime; dtTick: dtfunc.TDateTimeDelta; iCount: Integer;
...
function TICGrayBoxOPCHDANode.CalcEndDateTime(dtEnd: TDateTime;
                                       bNotMonth: Boolean; bNotDay: Boolean; bNotHour: Boolean; bNotMinute: Boolean; bNotSecond: Boolean): TDateTime;
var
  curYear, curMonth, curDay : Word;
  curHour, curMin, curSec, curMilli : Word;
